...
export const cycleStatusReply = (date: number) => {
  ...
  if (cycleDates.left === null) {
    ...
    const cycleEndDate = new Date(
      tempData()[cycleDates.right].endDate
    ).setHours(0, 0, 0, 0);
    if (cycleEndDate === date) {
      ...
      output = `${businessDays} work days left in ${
        tempData()[cycleDates.right].name
      }`;
    }
  }
  return output;
...
export const cycleInfoReply = (string: string | number | Date) => {
  let date: number;
  let output = "";
